...
第一版聚焦 CPU 和内存两个资源维度，提供物理机配置、任务配置、多场景测试集导入、调度算法选择、算法对比、离散时间仿真、指标统计和图表展示能力。项目采用前后端分离结构，后端使用 FastAPI，前端使用 Vue 3，机器与任务配置使用 SQLite 和原生 SQL 持久化。
...
- `best_fit` / `worst_fit` 当前基于 CPU 与内存剩余比例做双资源综合评分，不再只看单一 CPU 维度。
...
1. 启动后端和前端服务。
2. 打开前端页面。
3. 在“概览”页面选择一个测试集并一键导入数据。
4. 进入“物理机”或“任务”页面查看、编辑或补充配置。
5. 进入“仿真”页面，选择一种算法并设置最大仿真时间。
6. 点击“开始运行”。
7. 查看指标卡、资源利用率趋势图和任务时间线图。
8. 重点观察平均等待时间、最长等待时间、截止期违约率和负载均衡分数。
9. 进入“算法对比”页面，选择多个算法并查看横向对比结果与综合推荐。
...
这些数据包含更明显的异构机器、CPU 密集型任务、内存密集型任务、短任务、长任务、不同优先级和不同截止期，用于让不同算法在等待时间、最长等待时间、截止期违约率和负载均衡分数上产生更直观的差异。
...
导入测试集时会先清空对应类型的已有配置，再写入所选测试集，属于覆盖式导入，避免多次导入导致数据叠加。
如果需要手动清空，也可以使用：
...
- 后端测试通过：`cd backend && .venv/bin/pytest`（31 项通过）
